...
- *Fork* (~F~) opens a transient menu for forking the current repository on GitHub. By default the fork is created without cloning it or modifying your local git remotes. Toggle options to clone the fork, add a git remote (=--remote=, which sets the fork as =origin= and renames any existing =origin= to =upstream=), rename the fork, create it in an organization, or include only the default branch.
...
- *Set default repo* (~D~) offers the current repository, its fork parent, and the repository's GitHub remotes as completion candidates (defaulting to the fork parent). This configures which repository ~gh~ targets, and is often needed before other ~gh~ commands work in a repository with multiple remotes.
