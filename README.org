...
Features include:
  ...
  - Fork, create, and sync repositories, and set the default repository for ~gh~
...
| Key | Action                                                        |
|-----+---------------------------------------------------------------|
| =R= | View repo info (=C-u= for another repo)                       |
| =L= | List an owner's repositories                                  |
| =o= | Open the repo in the browser (=C-u= for another repo)         |
| =F= | Fork the current repo (=C-u= to also add a git remote)        |
| =N= | Create a new repository                                       |
| =S= | Sync a fork with its upstream (=C-u= for another repo)        |
| =D= | Set the default repo (=C-u= to view the current default)      |

Notes:

- *View repo info* (~R~) opens a dedicated buffer showing visibility, default branch, stars, forks, upstream (for forks), topics, and URLs.
- *Fork* (~F~) forks the current repository on GitHub without cloning it or modifying your local git remotes. With a prefix argument (=C-u=) it also adds a git remote for the fork, setting it as =origin= and renaming any existing =origin= remote to =upstream= (the default ~gh repo fork~ behavior).
- *New repo* (~N~) creates the repository non-interactively: it prompts for a name, visibility, and an optional description.
- *Set default repo* (~D~) offers the GitHub remotes of the current repository as completion candidates. This configures which repository ~gh~ targets, and is often needed before other ~gh~ commands work in a repository with multiple remotes.
...
*Note:* /This feature set represents the 1.0 release. I consider the current scope complete and intentionally avoid overlapping with [[https://github.com/magit/forge][Forge]] features. If you have ideas for additional functionality, please open an issue to discuss./
